...
/// These are day-unique and must be strictly increasing for each new number.
///
...
/// To manually get the next available number, send an AccountQuery request.
///
/// If you may need to refer to an order at a later time, 
...
impl UserRefNum {
    ...
    /// Fresh number to start the day, beginning at 1.
    pub fn new() -> Self {
        UserRefNum(1u32)
    }
    ...
    ///
    /// There is no way to initialize a UserRefNum with an arbitrary value,
    /// which is intentional, to prevent accidental reuse of identifiers.
    ///
